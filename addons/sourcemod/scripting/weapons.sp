...
public Action EventPlayerSpawn(Handle event, const char []name, bool dontBroadcast){
	...
	{
		if(g_iKnife[client] == 37)	//Butterfly
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 48)	//Classic
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 33)	//Karambit
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 34)	//M9 Bayonet
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 51)	//Nomad
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 49)	//Paracord
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 52)	//Skeleton
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 50)	//Survival
		{
			g_iKnife[client] = 0;
		}
		else if(g_iKnife[client] == 46)	//Talon
		{
			g_iKnife[client] = 0;
		}
	}
